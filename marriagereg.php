				...
				<h2>REGISTER</h2>
				 ...
				 <div class="col-md-6  register-top-grid">
					<h3>MARRIAGE REGISTRATION</h3>
                                        <form method="post" action="marriagereg_action.php">
					...
					<div>
						<span> Marriage Place</span>
                                                <input type="text" name="marriageplace" required pattern="[a-zA-Z\s]+"> 
					</div>
					<div>
					 ...
					 <div>
						 <span>Spouse Name</span>
                                                 ...
                                                 include 'connection.php';
                                                       $s="select spousename from engagement where memberid='$memberid'";
                                                       $s1=  mysqli_query($con,$s);
                                                       if($s2=  mysqli_fetch_assoc($s1))
                                                       {
                                                           $name=$s2['spousename'];
                                                       }
                                                       ?>
                                                 <input type="text" name="spousename" value="<?php echo $name;?>"> 
					</div>
				    ...
				    <div class="col-md-6  register-top-grid">
                                    ...
                                    <?php
                                    include 'connection.php';
                                    $s="select * from marriage where memberid='$memberid'";
                                    $s1=  mysqli_query($con, $s);
                                    while ($s2=mysqli_fetch_assoc($s1))
                                    {
                                        ?>
                                    <table border="1">
                                         <tr><th>MARRIAGE DATE</th><td><?php echo $s2['marriagedate'];?></td></tr>
                                        <tr><th>MARRIAGE PLACE</th><td><?php echo $s2['marriageplace'];?></td></tr>
